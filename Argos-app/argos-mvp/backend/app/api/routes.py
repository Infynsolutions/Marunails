...
All endpoints require a valid Supabase JWT via get_current_tenant().
Tenant isolation is enforced at DB level (RLS) + verified here.
"""
...
from app.api.auth import get_current_tenant, AuthContext
...
# ── Me (resolve current user's tenant) ──

@router.get("/me")
async def get_me(ctx: AuthContext = Depends(get_current_tenant)):
    """Return the current user's tenant and role. Frontend uses this after login."""
    return {
        "user_id": ctx.user_id,
...
async def get_dashboard(tenant_id: str, ctx: AuthContext = Depends(get_current_tenant)):
    """Get full dashboard data: KPIs, charts, breakdown, recent transactions."""
    if tenant_id != ctx.tenant_id:
        raise HTTPException(status_code=403, detail="Acceso denegado")

    try:
        return dash_service.get_dashboard(ctx.tenant_id)
    except Exception as e:
        logger.error("Dashboard error for %s: %s", ctx.tenant_id, e)
...
async def get_transactions(
    ...
    result = db.get_transactions(ctx.tenant_id, limit=limit, offset=offset)
    last_sync = db.get_last_sync(ctx.tenant_id)
...
async def chat_with_argos(
    ...
    try:
        ...
        logger.error("Chat error for %s: %s", ctx.tenant_id, e)
        raise HTTPException(
            status_code=500,
...
async def sync_sheets(
    ...
    if tenant_id != ctx.tenant_id:
        raise HTTPException(status_code=403, detail="Acceso denegado")
    if ctx.role != "owner":
        raise HTTPException(status_code=403, detail="Solo el dueño puede sincronizar datos")

    tenant = db.get_tenant(ctx.tenant_id)

    # New tenants use web-first data entry, not Sheets sync
    ...
    for sid in spreadsheet_ids:
        try:
            ...
            if rows:
                upsert_result = db.upsert_transactions(ctx.tenant_id, rows)
    ...
    db.log_sync(ctx.tenant_id, total_processed, total_new, total_updated, all_errors)
...
async def get_alerts(tenant_id: str, ctx: AuthContext = Depends(get_current_tenant)):
    """Get all alerts for a tenant with severity counts."""
    if tenant_id != ctx.tenant_id:
        raise HTTPException(status_code=403, detail="Acceso denegado")

    alerts = db.get_alerts(ctx.tenant_id)
...
async def resolve_alert(
    ...
    result = db.resolve_alert(alert_id, ctx.tenant_id)
    if not result:
...
async def create_transaction(
    tenant_id: str, req: TransactionCreate,
    ctx: AuthContext = Depends(get_current_tenant),
):
    ...
    if req.type.value == "ingreso" and req.product_id and req.quantity:
        try:
            ...
            logger.error("create_sale error for %s: %s", ctx.tenant_id, e)
    ...
    try:
        ...
        result = db.create_transaction(ctx.tenant_id, data)
        return result
    except Exception as e:
        logger.error("create_transaction error for %s: %s", ctx.tenant_id, e)
...
async def create_product(
    tenant_id: str, req: ProductCreate,
    ctx: AuthContext = Depends(get_current_tenant),
):
    if tenant_id != ctx.tenant_id:
        raise HTTPException(status_code=403, detail="Acceso denegado")
    try:
        return db.create_product(ctx.tenant_id, req.model_dump())
    except Exception as e:
        logger.error("create_product error for %s: %s", ctx.tenant_id, e)
...
async def list_products(tenant_id: str, ctx: AuthContext = Depends(get_current_tenant)):
    if tenant_id != ctx.tenant_id:
        raise HTTPException(status_code=403, detail="Acceso denegado")
    return db.get_products(ctx.tenant_id)


@router.get("/products/{tenant_id}/{sku}", response_model=ProductResponse)
async def get_product(
    tenant_id: str, sku: str,
    ctx: AuthContext = Depends(get_current_tenant),
):
    if tenant_id != ctx.tenant_id:
        raise HTTPException(status_code=403, detail="Acceso denegado")
    product = db.get_product_by_sku(ctx.tenant_id, sku)
    if not product:
...
async def add_stock_movement(
    tenant_id: str, req: StockMovementCreate,
    ctx: AuthContext = Depends(get_current_tenant),
):
    if tenant_id != ctx.tenant_id:
        raise HTTPException(status_code=403, detail="Acceso denegado")
    try:
        ...
        return db.create_stock_movement(ctx.tenant_id, data)
    except Exception as e:
        logger.error("stock_movement error for %s: %s", ctx.tenant_id, e)
...
async def get_stock(tenant_id: str, ctx: AuthContext = Depends(get_current_tenant)):
    if tenant_id != ctx.tenant_id:
        raise HTTPException(status_code=403, detail="Acceso denegado")
    products = db.get_products(ctx.tenant_id)
...
async def get_report(
    ...
    now = datetime.utcnow()
    try:
        return dash_service.get_report(ctx.tenant_id, year or now.year, month or now.month)
    except Exception as e:
        logger.error("Report error for %s: %s", ctx.tenant_id, e)
...
async def get_collections(tenant_id: str, ctx: AuthContext = Depends(get_current_tenant)):
    """Pending income transactions grouped by aging bucket."""
    if tenant_id != ctx.tenant_id:
        raise HTTPException(status_code=403, detail="Acceso denegado")
    try:
        return dash_service.get_collections_summary(ctx.tenant_id)
    except Exception as e:
        logger.error("Collections error for %s: %s", ctx.tenant_id, e)
